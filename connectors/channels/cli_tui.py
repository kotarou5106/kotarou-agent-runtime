...
from agent_runtime.config import DEFAULT_SOCKET, _normalize_cli_socket_endpoint
...
class HeaderBar(Horizontal):
    def compose(self) -> ComposeResult:
        yield Static("# Personal Agent OS CLI / Textual", id="title")
        yield Static("0 sent  0 recv  0 tools  connected: no", id="meta")


class FooterBar(Horizontal):
    def compose(self) -> ComposeResult:
        yield Static("Personal Agent OS (Textual TUI)", id="footer-left")
        yield Static(
...
def run_tui(socket_path: str = DEFAULT_SOCKET) -> None:
    mouse_enabled = _env_bool("kotarou_TUI_MOUSE", True)
