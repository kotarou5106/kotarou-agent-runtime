...
from agent_runtime.config_models import QQGroupConfig
...
from connectors.channels.base import AttachmentStore, SessionIdentityIndex
from connectors.channels.group_filter import (
    DefaultGroupFilter,
    GroupMessageFilter,
    strip_at_segments,
)
from agent_runtime.core.net.http import HttpRequester, RequestBudget, get_default_http_requester
from storage.sessions.manager import SessionManager

# NcatBot 运行时产物（plugins、logs）放到用户目录，不污染项目目录
_NCATBOT_DIR = Path.home() / ".kotarou" / "ncatbot"
...
_TRACE_DEFAULT_ACTOR = "Kotarou"


def _ensure_ncatbot_runtime_dir(preferred: Path) -> Path:
    try:
        preferred.mkdir(parents=True, exist_ok=True)
        return preferred
    except OSError as exc:
        fallback = Path(tempfile.gettempdir()) / "kotarou" / "ncatbot"
        logger.warning(
            "[qq] cannot use ncatbot runtime dir %s: %s; fallback=%s",
            preferred,
            exc,
            fallback,
        )
...
def _patch_ncatbot_ws_open_timeout(timeout_seconds: float) -> None:
    ...
    try:
        ...
        original_connect = getattr(
            adapter_mod,
            "_kotarou_original_websockets_connect",
            None,
        )
        if original_connect is None:
            original_connect = adapter_mod.websockets.connect
            adapter_mod._kotarou_original_websockets_connect = original_connect

            def _patched_connect(*args, **kwargs):
                configured_timeout = getattr(
                    adapter_mod,
                    "_kotarou_websocket_open_timeout_seconds",
                    None,
                )
                if configured_timeout is not None:
                    kwargs["open_timeout"] = configured_timeout
                return adapter_mod._kotarou_original_websockets_connect(*args, **kwargs)

            adapter_mod.websockets.connect = _patched_connect

        adapter_mod._kotarou_websocket_open_timeout_seconds = timeout_seconds
...
class QQChannel:
    ...
    def __init__(
        ...
        # Kotarou 只需要 NapCat 的 OneBot WebSocket，禁用 WebUI 避免启动时卡交互 token。
        ncatbot_config.napcat.enable_webui = False
        ncatbot_config.enable_webui_interaction = False
        # 运行时产物重定向到 Kotarou runtime 目录，不污染项目目录。
        ncatbot_dir = _ensure_ncatbot_runtime_dir(
            (self._workspace / "ncatbot") if self._workspace else _NCATBOT_DIR
        )
