...
Usage:
  # Full run:
  python -m evaluation_system.longmemeval.run \\
      --config evaluation_system/longmemeval/config.toml \\
      --data evaluation_system/longmemeval/data/<data-file>.json \\
      --workspace /tmp/lme_bench

  # 2 concurrent workers:
  python -m evaluation_system.longmemeval.run ... --workers 2

  # Resume (skip already-ingested):
  python -m evaluation_system.longmemeval.run ... --resume

  # QA only / ingest only:
  python -m evaluation_system.longmemeval.run ... --qa-only
  python -m evaluation_system.longmemeval.run ... --ingest-only

  # Smoke-test with first N instances:
  python -m evaluation_system.longmemeval.run ... --limit 5

Results are written to --output (default: evaluation_system/longmemeval/results/<timestamp>.json).
"""
...
logger = logging.getLogger("evaluation_system.longmemeval")


def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Run LongMemEval benchmark against the kotarou agent runtime."
    )
    p.add_argument("--config", required=True, type=Path, help="Path to config.toml")
    p.add_argument("--data", required=True, type=Path,
                   help="Path to longmemeval_kotarou.json")
    p.add_argument("--workspace", type=Path, default=Path("/tmp/lme_bench"),
                   help="Workspace directory (created on first run)")
    p.add_argument("--output", type=Path, default=None,
                   help="Output JSON (default: evaluation_system/longmemeval/results/<ts>.json)")
