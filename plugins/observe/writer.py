...
from .events import MemoryWriteTrace, RagQueryLog, TraceReplayLog, TurnTrace
...
class TraceWriter:
    def __init__(self, db_path: Path) -> None:
        ...
        self._queue: asyncio.Queue[
            TurnTrace | RagQueryLog | MemoryWriteTrace | TraceReplayLog
        ] = asyncio.Queue(
            maxsize=_QUEUE_MAX
        )
        self._dropped = 0
    ...
    def emit(self, event: TurnTrace | RagQueryLog | MemoryWriteTrace | TraceReplayLog) -> None:
    ...
    def _write_one(
        self, conn, event: TurnTrace | RagQueryLog | MemoryWriteTrace | TraceReplayLog
    ) -> None:
        ts = _now_iso()
        ...
        elif isinstance(event, TraceReplayLog):
            _write_trace_replay(conn, event, ts)
...
def _write_trace_replay(conn, e: TraceReplayLog, ts: str) -> None:
    with conn:
        conn.execute(
            """
            INSERT INTO trace_events (
                trace_id, ts, source, session_key, channel, chat_id,
