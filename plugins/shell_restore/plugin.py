...
def _restore_dir() -> str:
    configured = os.environ.get("KOTAROU_RESTORE_DIR")
    if configured:
        return configured
    preferred = Path.home() / ".kotarou" / "restore"
    try:
        preferred.mkdir(parents=True, exist_ok=True)
        return str(preferred)
    except OSError as exc:
        fallback = Path(tempfile.gettempdir()) / "kotarou" / "restore"
        logger.warning(
            "[%s] cannot use restore dir %s: %s; fallback=%s",
            "shell_restore",
            preferred,
            exc,
            fallback,
