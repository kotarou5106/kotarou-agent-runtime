...
DEFAULT_SOCKET = "127.0.0.1:8765" if os.name == "nt" else "/tmp/kotarou.sock"
...
def load_config(path: str | Path = "config.toml") -> Config:
    ...
    knowledge = _load_knowledge_config(data)
...
def _load_knowledge_config(data: dict) -> KnowledgeConfig:
    raw = _as_dict(data.get("knowledge"))
    return KnowledgeConfig(
        enabled=bool(raw.get("enabled", False)),
        db_path=str(raw.get("db_path", "knowledge.db") or "knowledge.db"),
        top_k=int(raw.get("top_k", 6)),
        score_threshold=float(raw.get("score_threshold", 0.20)),
        chunk_max_chars=int(raw.get("chunk_max_chars", 1200)),
        chunk_overlap_chars=int(raw.get("chunk_overlap_chars", 120)),
        inject_max_chars=int(raw.get("inject_max_chars", 2400)),
    )
...
def _resolve(value: str) -> str:
    ...
    if m:
        key_file = Path.home() / ".kotarou" / "workspace" / "memory" / m.group(1)
        if key_file.exists():
