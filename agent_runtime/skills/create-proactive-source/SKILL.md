...
### 1. MCP Server 注册（`$HOME/.kotarou/workspace/mcp_servers.json`）
...
### 2. Proactive Source 注册（`$HOME/.kotarou/workspace/proactive_sources.json`）
...
- 读取 `$HOME/.kotarou/workspace/mcp_servers.json`，列出所有已注册的 server 名称
- 读取 `$HOME/.kotarou/workspace/proactive_sources.json`，列出已接入的 source 条目
- 浏览 `$HOME/.kotarou/workspace/mcp/` 目录，了解每个 server 的用途
...
- 如果新建 server：在 `$HOME/.kotarou/workspace/mcp/` 下创建目录，用 FastMCP 实现标准协议工具，创建 `run_mcp.py` 入口和虚拟环境
...
如果需要导入当前 agent 仓库里的 `proactive_system.*` 模块，不要写死绝对路径；先定位仓库根目录，再把它加入 `sys.path`。
...
# 检查点 2: 模拟 engine 提取
from proactive_system.mcp_sources import _extract_proactive_events
...
# 检查点 3: 模拟 engine 归一化
from proactive_system.contracts import normalize_alert
for e in events:
...
from proactive_system.contracts import normalize_content
for e in events:
...
from proactive_system.mcp_sources import _extract_context_items
...
from proactive_system.contracts import normalize_context
for item in items:
...
- MCP server 文件放在 `$HOME/.kotarou/workspace/mcp/<server-name>/`，不要放到仓库内
