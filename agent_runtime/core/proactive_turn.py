...
from agent_runtime.prompting import (
    PromptSectionRender,
    build_context_frame_content,
    build_context_frame_message,
)
from agent_runtime.tool_hooks import ToolExecutionRequest, ToolExecutor, ToolHook
from agent_runtime.turns.orchestrator import TurnOrchestrator
from agent_runtime.turns.result import TurnOutbound, TurnResult, TurnTrace
from agent_runtime.core.memory.markdown import MemoryProfileApi
from proactive_system.config import ProactiveConfig
from proactive_system.context import AgentTickContext
from proactive_system.contracts import (
    normalize_alert,
    normalize_content,
    normalize_context,
)
from agent_runtime.core.drift_turn import DriftTurnPipeline
from proactive_system.gateway import DataGateway, GatewayDeps, GatewayResult
from proactive_system.tools import TOOL_SCHEMAS, ToolDeps, dispatch
...
def _log_content_candidates(gw: GatewayResult) -> None:
    if not gw.content_meta:
        logger.info("[proactive_system] content candidates: 0")
        return
    lines: list[str] = []
    for index, item in enumerate(gw.content_meta, 1):
        ...
        lines.append(line)
    logger.info(
        "[proactive_system] content candidates: %d\n%s",
        len(gw.content_meta),
        "\n".join(lines),
    )
...
class ProactiveTurnPipeline:
    ...
    def _gate_check(self, ctx: AgentTickContext) -> GateResult:
        ...
        if not str(self._cfg.default_chat_id or "").strip():
            logger.debug("[proactive_system] gate: no chat_id → blocked")
        ...
        if self._passive_busy_fn and self._passive_busy_fn(self._session_key):
            logger.debug("[proactive_system] gate: passive_busy → blocked")
        ...
        if self._state_store.count_deliveries_in_window(
            ...
            logger.debug("[proactive_system] gate: delivery_cooldown → blocked")
        ...
        if self._any_action_gate is not None:
            ...
            if not should_act:
                logger.debug("[proactive_system] gate: anyaction → blocked meta=%s", meta)
        ...
        if not gw_result.alerts and not gw_result.content_meta and not ctx.context_as_fallback_open:
            if self._drift_pipeline is not None and self._cfg.drift_enabled:
                ...
                if (
                    ...
                    logger.info(
                        "[proactive_system] fetch: drift blocked by interval last_drift_at=%s min_interval_hours=%d",
                ...
                logger.info("[proactive_system] fetch: empty gateway, attempting drift")
                ...
                if entered_drift:
                    ...
                    logger.info("[proactive_system] fetch: drift entered, message_sent=%s", ctx.drift_message_sent)
                    self.last_ctx = ctx
                    return FeedResult(drift_entered=True, base_score=0.0)
                logger.info("[proactive_system] fetch: drift not entered")
            logger.info("[proactive_system] fetch: no data and fallback off → skip")
            ctx.terminal_action = "skip"
            ctx.skip_reason = "no_content"
            self.last_ctx = ctx
            return FeedResult(drift_entered=False, base_score=None)
    ...
    async def _judge_evaluate(self, ctx: AgentTickContext, messages: list[dict]) -> None:
        ...
        if ctx.terminal_action == "skip" and gw_result is not None and gw_result.content_meta:
            ...
            if unclassified_ids:
                ...
                logger.info(
                    "[proactive_system] judge completeness: %d unclassified, resetting → %s",
        ...
        if ctx.terminal_action is None and ctx.interesting_item_ids and ctx.steps_taken < self._cfg.agent_tick_max_steps:
            ...
            logger.info("[proactive_system] judge reflection: interesting=%d, injecting prompt", len(ctx.interesting_item_ids))
    ...
    async def _resolve_decide(self, ctx: AgentTickContext) -> ResolveResult:
        ...
        if ctx.terminal_action != "reply":
            logger.info(
                "[proactive_system] resolve: action=%s steps=%d discarded=%d interesting=%d skip_reason=%s note=%s",
        ...
        if self._state_store.is_delivery_duplicate(
            ...
            logger.info("[proactive_system] resolve: delivery_dedupe hit")
        ...
        if self._cfg.message_dedupe_enabled and self._deduper is not None:
            ...
            if is_dup:
                logger.info("[proactive_system] resolve: message_dedupe hit: %s", reason)
                return ResolveResult(
                    action="skip",
                    result=TurnResult(
                        decision="skip",
    ...
    def _finalize_after_drift(self, ctx: AgentTickContext) -> None:
        ...
        if self._any_action_gate is not None:
            self._any_action_gate.record_action(now_utc=ctx.now_utc)
        logger.info(
            "[proactive_system] drift entered, skipping normal post_loop message_sent=%s finished=%s",
    ...
    async def _run_tool_step(
        ...
        *,
        ...
        if tool_call is None:
            logger.warning(
                "[proactive_system] %s: llm_fn returned None at step %d, stopping",
                loop_tag,
                ctx.steps_taken,
            )
            return False
        ...
        logger.info(
            "[proactive_system] %s step %d: %s  args=%s",
            loop_tag,
            ctx.steps_taken,
            tool_name,
            arg_summary,
        ...
        if exec_result.status == "error":
            logger.warning("[proactive_system] %s: tool error: %s", loop_tag, exec_result.output)
    ...
    def _build_system_prompt(self) -> str:
        from agent_runtime.identity import KOTAROU_IDENTITY, KOTAROU_PERSONALITY_RULES
        return (
            f"{KOTAROU_IDENTITY}\n\n"
            f"{KOTAROU_PERSONALITY_RULES}\n\n"
