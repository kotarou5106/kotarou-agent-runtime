...
import agent_runtime.core.passive_support as support
...
from agent_runtime.prompting import DEFAULT_CONTEXT_TRIM_PLANS, is_context_frame
from agent_runtime.provider import ContentSafetyError, ContextLengthError
from agent_runtime.retrieval.protocol import RetrievalRequest
from agent_runtime.tool_hooks import ToolExecutionRequest, ToolExecutor
...
from agent_runtime.turns.outbound import OutboundDispatch, OutboundPort
...
from agent_runtime.lifecycle.phase import Phase
from agent_runtime.lifecycle.phases.after_reasoning import (
    AfterReasoningFrame,
    default_after_reasoning_modules,
)
from agent_runtime.lifecycle.phases.after_step import AfterStepFrame, default_after_step_modules
from agent_runtime.lifecycle.phases.after_turn import AfterTurnFrame, default_after_turn_modules
from agent_runtime.lifecycle.phases.before_reasoning import (
    BeforeReasoningFrame,
    default_before_reasoning_modules,
)
from agent_runtime.lifecycle.phases.before_step import BeforeStepFrame, default_before_step_modules
from agent_runtime.lifecycle.phases.before_turn import BeforeTurnFrame, default_before_turn_modules
from agent_runtime.lifecycle.phases.prompt_render import (
    PromptRenderFrame,
    default_prompt_render_modules,
)
from agent_runtime.lifecycle.types import (
    AfterReasoningInput,
...
if TYPE_CHECKING:
    ...
    from agent_runtime.core.runtime_support import SessionLike, TurnRunResult
    from agent_runtime.looping.ports import LLMConfig, LLMServices, SessionServices
...
class PassiveTurnPipeline:
    ...
    async def run(
        ...
        *,
        ...
        trace_id = f"{key}:{uuid.uuid4().hex[:12]}"
        state.extra_metadata["trace_id"] = trace_id
        await self._emit_trace(
            ...
            title="Turn started",
            payload={"content": msg.content, "media_count": len(msg.media or [])},
        )
        ...
        try:
            ...
            await self._emit_trace(
                ...
                payload={
                    "history_count": len(before_turn.history_messages),
                    "skill_names": list(before_turn.skill_names),
                    "retrieved_memory_block": before_turn.retrieved_memory_block,
                    "retrieval_trace_raw": before_turn.retrieval_trace_raw,
                    "extra_hints": list(before_turn.extra_hints),
                    "abort": before_turn.abort,
                },
            )
            # TurnState 存内部默认 metadata；BeforeTurnCtx 存插件导出，同名 key 以后者覆盖。
            if before_turn.knowledge_context_block:
                state.extra_metadata["knowledge_context_block"] = before_turn.knowledge_context_block
            if before_turn.knowledge_citations:
                state.extra_metadata["knowledge_citations"] = list(before_turn.knowledge_citations)
            if before_turn.knowledge_retrieval_trace:
                state.extra_metadata["knowledge_retrieval_trace"] = dict(before_turn.knowledge_retrieval_trace)
            ...
            turn_result = await self._reasoner.run_turn(
                ...
                knowledge_context_block=before_reasoning.knowledge_context_block,
                knowledge_citations=list(before_reasoning.knowledge_citations),
                knowledge_retrieval_trace=dict(before_reasoning.knowledge_retrieval_trace),
...
class DefaultContextStore(ContextStore):
    def __init__(
        ...
        *,
        ...
        context: "ContextBuilder",
        knowledge_retrieval: "KnowledgeRetrievalPipeline | None" = None,
        history_window: int = 500,
    ) -> None:
    ...
    async def prepare(
        ...
        *,
        ...
        if self._knowledge_retrieval is not None:
            from agent_runtime.retrieval.knowledge_pipeline import (
                KnowledgeRetrievalRequest,
            )

            try:
                knowledge_result = await self._knowledge_retrieval.retrieve(
                    KnowledgeRetrievalRequest(
                        message=msg.content,
                        session_key=session_key,
                        channel=msg.channel,
                ...
                knowledge_block = knowledge_result.block
                knowledge_citations = list(knowledge_result.citations)
                knowledge_trace = dict(knowledge_result.trace)
            except Exception:
                knowledge_block = ""
                knowledge_citations = []
                knowledge_trace = {}
        ...
        return ContextBundle(
            ...
            knowledge_context_block=knowledge_block,
...
class DefaultReasoner(Reasoner):
    ...
    async def run_turn(
        ...
        *,
        ...
        from agent_runtime.core.runtime_support import TurnRunResult
        ...
        for attempt, plan in enumerate(attempts):
            ...
            prompt_render = await self.render_prompt(
                PromptRenderInput(
                    ...
                    knowledge_context_block=knowledge_context_block,
                    knowledge_citations=list(knowledge_citations or []),
                    knowledge_retrieval_trace=dict(knowledge_retrieval_trace or {}),
                )
            )
            ...
            await self._emit_trace(
                ...
                payload={
                    ...
                    "history_count": len(history_for_attempt),
        ...
        *,
        ...
        while True:
            ...
            await self._emit_trace(
                ...
                chat_id=tool_event_chat_id,
                phase="llm",
                event_type="llm.step.started",
                title=f"LLM step {iteration + 1} started",
                payload={
                    "iteration": iteration + 1,
                    "message_count": len(messages),
                    "input_tokens_estimate": step_ctx.input_tokens_estimate,
                    "visible_tool_names": sorted(visible_names) if visible_names is not None else None,
                    "schema_names": sorted(schema_names) if isinstance(schema_names, set) else schema_names,
                },
            )
            ...
            await self._emit_trace(
                ...
                chat_id=tool_event_chat_id,
                phase="llm",
                event_type="llm.step.completed",
                title=f"LLM step {iteration + 1} completed",
                payload={
                    "iteration": iteration + 1,
