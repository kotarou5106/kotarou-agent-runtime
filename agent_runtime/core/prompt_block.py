...
from agent_runtime.prompting import PromptSectionMeta, PromptSectionRender, SectionCache
from agent_runtime.prompts.agent import (
...
logger = logging.getLogger("agent_runtime.core.prompt_block")
...
#                              来源：agent_runtime/prompts/agent.py 里的固定行为规范
#                              时机：仅代码或 workspace 变化时才变，最稳定
#  20 SkillsCatalogPromptBlock → skills.build_skills_summary()
#                              来源：agent_runtime/skills/ 目录扫描结果、技能描述、依赖可用性
...
#  55 MemoryBlockPromptBlock   → 本轮个人长期记忆语义检索注入
#                              来源：retrieved_memory_block
#                              时机：每轮 retrieval 结果都可能不同，最高频
#  56 KnowledgePromptBlock     → 本轮文档知识库检索注入
#                              来源：knowledge_context_block
#                              时机：每轮 Document RAG 结果都可能不同，最高频
# ─────────────────────────────────────────────────────────────────────────────
...
class KnowledgePromptBlock:
    priority = 56
    label = "knowledge_context"
    is_static = False

    def render(self, ctx: TurnContext, cached_signature: str | None = None) -> str | None:
        block = (ctx.knowledge_context_block or "").strip()
        return block or None
