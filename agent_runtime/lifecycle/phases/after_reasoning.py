...
from agent_runtime.core.passive_support import update_session_runtime_metadata
from agent_runtime.core.response_parser import parse_response
from agent_runtime.lifecycle.phase import (
    PhaseFrame,
    PhaseModule,
...
from agent_runtime.events.events import OutboundMessage
from knowledge_system.injection import validate_answer_citations

if TYPE_CHECKING:
    from agent_runtime.looping.ports import SessionServices
    from storage.sessions.manager import Session
...
class _BuildAfterReasoningCtxModule:
    ...
    async def run(self, frame: AfterReasoningFrame) -> AfterReasoningFrame:
        ...
        citation_validation = validate_answer_citations(
            parsed.clean_text,
            _knowledge_citations_from_state(input.state.extra_metadata),
        )
        if citation_validation.warnings:
            logger.warning(
                "knowledge citation validation warnings session=%s warnings=%s",
                input.state.session_key,
                citation_validation.warnings,
            )
            input.state.extra_metadata["knowledge_citation_warnings"] = list(
                citation_validation.warnings
            )
        if citation_validation.used_ids:
            input.state.extra_metadata["knowledge_citation_used_ids"] = list(
                citation_validation.used_ids
            )
            ...
            outbound_metadata={
                ...
                **input.state.extra_metadata,
                ...
                "knowledge_citation_validation": {
                    ...
                    "used_ids": citation_validation.used_ids,
                    "valid_ids": citation_validation.valid_ids,
                    "missing_ids": citation_validation.missing_ids,
                    "warnings": citation_validation.warnings,
                },
            },
        )
        return frame
...
class _PersistUserMessageModule:
    ...
    async def run(self, frame: AfterReasoningFrame) -> AfterReasoningFrame:
        ...
        trace_id = state.extra_metadata.get("trace_id")
...
class _PersistAssistantMessageModule:
    ...
    async def run(self, frame: AfterReasoningFrame) -> AfterReasoningFrame:
        ...
        trace_id = frame.input.state.extra_metadata.get("trace_id")
        if isinstance(trace_id, str) and trace_id:
            assistant_kwargs["trace_id"] = trace_id
        knowledge_citations = frame.input.state.extra_metadata.get("knowledge_citations")
        if isinstance(knowledge_citations, list) and knowledge_citations:
            assistant_kwargs["knowledge_citations"] = knowledge_citations
        citation_warnings = frame.input.state.extra_metadata.get("knowledge_citation_warnings")
        if isinstance(citation_warnings, list) and citation_warnings:
            assistant_kwargs["knowledge_citation_warnings"] = citation_warnings
...
def _knowledge_citations_from_state(metadata: dict[str, Any]) -> list[dict[str, object]]:
