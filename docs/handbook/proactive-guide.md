...
Personal Agent OS 不只是被动回复——它每 N 秒/分钟跑一轮 tick，检查你订阅的信息源，自主决定"现在该不该给你发消息、发什么"。没有内容可推时就进入 **Drift 空闲任务**。
...
2. **一段固定 system prompt**（硬编码在 `proactive_system/agent_tick.py:_build_system_prompt()`）
...
### 2. 注册 MCP server（`~/.kotarou/workspace/mcp_servers.json`）
...
### 3. 声明信息源（`~/.kotarou/workspace/proactive_sources.json`）
...
### 4. 写推送规则（`~/.kotarou/workspace/PROACTIVE_CONTEXT.md`）
...
**真实实现**：`~/.kotarou/workspace/mcp/fitbit-mcp/src/mcp_bridge.py`：
...
**真实实现**：`~/.kotarou/workspace/mcp/feed-mcp/src/feed_backend.py:1284`：
...
**真实实现**：
- fitbit sleep：`~/.kotarou/workspace/mcp/fitbit-mcp/src/mcp_bridge.py:136`
- steam game：`~/.kotarou/workspace/mcp/steam-mcp/steam_proactive.py`
