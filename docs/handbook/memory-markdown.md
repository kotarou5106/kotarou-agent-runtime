...
Personal Agent OS 的记忆分为两层：**Markdown 文件层**（人类可读，LLM 直接写入）和**向量数据库层**（`memory_system.db`，语义检索）。本文档只讲 Markdown 层——哪几个文件、各自干什么、consolidation 怎么把对话变成记忆。

---

## 五个 Markdown 文件

都在 `~/.kotarou/workspace/memory/` 下：

| 文件 | 写者 | 读方 | 用途 |
|------|------|------|------|
| **MEMORY.md** | Optimizer（主 agent 自动维护） | 被动/主动 agent 的 system prompt | 长期记忆——用户的稳定事实、偏好、身份 |
| **SELF.md** | Optimizer（主 agent 自动维护） | 被动/主动 agent 的 system prompt | Agent 的自我认知——形象、对用户的理解、关系定义 |
...
|----------|------|---------|---------|
| 30 | SelfModel | `SELF.md` | `## Kotarou 自我认知\n\n{全文}` |
...
两层记忆的分工：**Markdown 层**管"人类能看懂的全景"——你是谁、你喜欢什么、最近发生了什么。**向量层**管"机器能搜到的细节"——语义检索、时间过滤、去重计数。两个层各自独立更新，通过 `ConsolidationCommitted` 事件桥接——markdown 层写完 HISTORY 后发事件，向量层收到后把同批数据 embed 写入 `memory_system.db`。

---

## 向量记忆 API——谁在调它

`agent_runtime/core/memory/engine.py` 定义了一套抽象协议（`MemoryEngine`），由四个子协议组成。引擎本身是一个 **plugin**——`[memory].engine` 配置项指定用哪个实现，留空 = `default_memory` 插件。协议与实现解耦。
...
| 位置 | 行 | 方法 | 说明 |
|------|-----|------|------|
| `agent_runtime/retrieval/default_pipeline.py` | 31 | `retrieve()` | 被动 turn 每轮的语义检索入口。`DefaultContextStore.prepare()` → `retrieval_pipeline.retrieve()` → `engine.retrieve(MemoryEngineRetrieveRequest)` |
| `agent_runtime/tools/recall_memory.py` | 138 | `retrieve_explicit()` | `recall_memory` 工具。LLM 传入 query/memory_type/time_filter/limit，工具调 `retrieve_explicit(ExplicitRetrievalRequest)` |
| `proactive_system/tools.py` | 220 | `retrieve_interest_block()` | proactive tick 中评估内容候选时调用，查询用户对某条内容是否可能感兴趣 |
| `agent_runtime/tools/memorize.py` | 71 | `remember()` | `memorize` 工具。用户要求"记住..."时调 `engine.remember(RememberRequest)` |
| `agent_runtime/tools/forget_memory.py` | 55 | `forget()` | `forget_memory` 工具。用户纠正错误记忆时调 `memory.forget(ForgetRequest)` |
| `dashboard/backend/api.py` | 1032 | `list_items_for_dashboard()` | Dashboard 记忆列表页 |
| `dashboard/backend/api.py` | 1063 | `find_similar_items_for_dashboard()` | Dashboard 相似记忆查询 |
| `dashboard/backend/api.py` | 1085 | `get_item_for_dashboard()` | Dashboard 单条记忆详情 |
| `dashboard/backend/api.py` | 1099 | `update_item_for_dashboard()` | Dashboard 编辑记忆 |
| `dashboard/backend/api.py` | 1115 | `delete_item()` | Dashboard 删除单条 |
| `dashboard/backend/api.py` | 1122 | `delete_items_batch()` | Dashboard 批量删除 |
| `agent_runtime/core/memory/runtime.py` | 51/57/63 | `retrieve()` / `retrieve_explicit()` / `retrieve_interest_block()` | `MemoryRuntime` 薄封装层，统一入口 |
| `agent_runtime/core/memory/plugin.py` | 80-126 | 全部方法 | `DisabledMemoryEngine` — 记忆关闭时的空实现，所有方法都返回空/报错 |
...
bootstrap/memory.py:116 → engine = plugin_runtime.engine
  ...
  proactive_system/tools.py:38 → ToolDeps.memory → engine.retrieve_interest_block()
```
