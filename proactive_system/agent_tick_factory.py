...
from agent_runtime.skills import BUILTIN_SKILLS_DIR
from agent_runtime.tool_hooks import ToolHook
...
from proactive_system import mcp_sources
...
from proactive_system.drift_tools import DriftToolDeps
...
class AgentTickFactory:
    ...
    def build(self) -> ProactiveTurnPipeline:
        if self._deps.pool is None:
            raise RuntimeError("proactive_system 依赖 MCP 连接池，pool 不能为空")
    ...
    def _build_llm_fn(self) -> LlmFn:
        ...
        async def llm_fn(
            ...
            if not resp.tool_calls:
                ...
                logger.warning(
                    "[proactive_system] llm_fn: no tool call returned (text=%r)",
    ...
    def _build_tool_deps(self) -> ToolDeps:
        ...
        try:
            ...
            logger.warning("[proactive_system] web_fetch 不可用，已降级禁用: %s", e)
        return ToolDeps(
