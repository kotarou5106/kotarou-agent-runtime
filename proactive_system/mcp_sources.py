"""
proactive_system/mcp_sources.py — 从 MCP server 拉取 ProactiveEvent 的通用客户端。

读取 ~/.kotarou/workspace/proactive_sources.json 中的配置，
动态调用各 MCP server 的 get_tool / ack_tool。

使用项目自带的 agent_runtime.mcp.client.McpClient，无需额外依赖。
"""
...
if TYPE_CHECKING:
    from proactive_system.event import AlertEvent

logger = logging.getLogger(__name__)

_DEFAULT_WORKSPACE = Path.home() / ".kotarou" / "workspace"
