...
from agent_runtime.core.common.timekit import parse_iso
from connectors.persistence.json_store import atomic_save_json, load_json
...
class DriftStateStore:
    def __init__(
        ...
        *,
        ...
        self.builtin_skills_dir = self._resolve_builtin_skills_dir(builtin_skills_dir)
        self.include_builtin_skills = include_builtin_skills
        self.builtin_skill_names = set(builtin_skill_names or set())
        self.skills_dir.mkdir(parents=True, exist_ok=True)

    @staticmethod
    def _resolve_builtin_skills_dir(path: Path | None) -> Path | None:
        if path is None:
            return None
        expanded = path.expanduser()
        has_skills = expanded.exists() and any(expanded.glob("*/SKILL.md"))
        if has_skills:
            return expanded
        bundled = Path(__file__).resolve().parents[1] / "agent_runtime" / "skills"
        if expanded == Path("skills") and bundled.exists():
            return bundled
        return expanded
