...
from proactive_system.memory_optimizer import MemoryOptimizerBusy
...
def _observe_db_path(workspace: Path) -> Path:
    return workspace / "observe" / "observe.db"
...
def _read_trace_list(
    ...
    *,
    ...
    db_path = _observe_db_path(workspace)
    ...
    with sqlite3.connect(str(db_path)) as conn:
        ...
        try:
            total_row = conn.execute(
                f"SELECT COUNT(1) AS c FROM (SELECT trace_id FROM trace_events {where} GROUP BY trace_id)",
                tuple(params),
            ).fetchone()
            rows = conn.execute(
                f"""
                SELECT
                    ...
                    MAX(CASE WHEN event_type = 'turn.committed' THEN 1 ELSE 0 END) AS committed
                FROM trace_events
                {where}
                GROUP BY trace_id, session_key
...
def _read_trace_detail(workspace: Path, trace_id: str) -> dict[str, Any] | None:
    db_path = _observe_db_path(workspace)
    if not db_path.exists():
        return None
    with sqlite3.connect(str(db_path)) as conn:
        ...
        try:
            rows = conn.execute(
                """
                SELECT id, trace_id, ts, source, session_key, channel, chat_id,
                       phase, event_type, title, payload_json
                FROM trace_events
                WHERE trace_id = ?
                ORDER BY id ASC
                """,
...
def create_dashboard_app(
    ...
    *,
    ...
    knowledge_store = KnowledgeStore(workspace / "knowledge.db")
    ...
    app = FastAPI(title="Kotarou Dashboard API", lifespan=lifespan)
    ...
    @app.get("/api/dashboard/trace-replay")
    def list_trace_replays(
        session_key: str | None = None,
        page: int = 1,
        page_size: int = 25,
    ) -> dict[str, Any]:
        items, total = _read_trace_list(
            workspace,
            session_key=session_key,
            page=page,
            page_size=page_size,
    ...
    @app.get("/api/dashboard/trace-replay/{trace_id:path}")
    def get_trace_replay(trace_id: str) -> dict[str, Any]:
        item = _read_trace_detail(workspace, trace_id)
        if item is None:
    ...
    def list_knowledge_documents(status: str = "active") -> dict[str, Any]:
        items = knowledge_store.list_documents(status=status)
        return {
            "items": items,
            "total": len(items),
            "vector_backend": knowledge_store.vector_backend,
        }

    @app.get("/api/dashboard/knowledge/documents/{document_id:path}")
    def get_knowledge_document(document_id: str) -> dict[str, Any]:
        item = knowledge_store.get_document(document_id)
        if item is None:
            raise HTTPException(status_code=404, detail="Knowledge document not found")
        return item

    @app.get("/api/dashboard/knowledge/chunks")
    def list_knowledge_chunks(
        document_id: str = "",
        limit: int = Query(100, ge=1, le=500),
    ) -> dict[str, Any]:
        items = knowledge_store.list_chunks(document_id=document_id, limit=limit)
        return {"items": items, "total": len(items)}

    @app.get("/api/dashboard/knowledge/retrieval-events")
    def list_knowledge_retrieval_events(
        limit: int = Query(100, ge=1, le=500),
    ) -> dict[str, Any]:
        items = knowledge_store.list_retrieval_events(limit=limit)
        return {"items": items, "total": len(items)}
