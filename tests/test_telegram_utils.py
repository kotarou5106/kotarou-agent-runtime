...
async def test_outbound_limiter_retries_after_cooling_down(monkeypatch):
    from connectors.channels import telegram_utils as mod
    ...
    monkeypatch.setattr("connectors.channels.telegram_utils.asyncio.sleep", sleep_mock)
    calls = 0
...
async def test_send_markdown_falls_back_to_plain_text(monkeypatch):
    ...
    monkeypatch.setattr(
        "connectors.channels.telegram_utils.convert_with_segments", fake_convert_with_segments
    )
...
async def test_stream_message_retry_after_enters_cooldown_without_blocking(monkeypatch):
    ...
    monkeypatch.setattr("connectors.channels.telegram_utils.asyncio.sleep", sleep_mock)
    monkeypatch.setattr(
        "connectors.channels.telegram_utils.asyncio.get_running_loop",
        lambda: _Loop(),
    )
