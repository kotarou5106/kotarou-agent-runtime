...
Covers:
  ...
  Item 2a — session metadata update behavior
    ...
    agent_runtime/core/passive_support.py.

  Item 2b — history type conversion
    Canonical types live in agent_runtime/core/types.py.
"""
...
def test_to_tool_call_groups_empty():
    ...
    from agent_runtime.core.types import to_tool_call_groups
...
def test_to_tool_call_groups_basic():
    ...
    from agent_runtime.core.types import to_tool_call_groups, ToolCallGroup, ToolCall
...
def test_history_message_fields():
    ...
    from agent_runtime.core.types import HistoryMessage
...
def test_turn_types_shim_is_removed():
    """looping.turn_types 已移除，核心类型统一从 agent_runtime.core.types 导入。"""
    import importlib.util

    assert importlib.util.find_spec("agent_runtime.looping.turn_types") is None


def test_core_boundary_modules_do_not_import_looping_turn_types():
    """Canonical agent runtime contracts must not depend on looping.turn_types."""
    from agent_runtime.core import passive_turn
    from agent_runtime.retrieval import protocol as retrieval_protocol

    assert "agent_runtime.looping.turn_types" not in inspect.getsource(passive_turn)
    assert "agent_runtime.looping.turn_types" not in inspect.getsource(retrieval_protocol)
