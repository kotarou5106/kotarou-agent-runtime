...
from agent_runtime.memory import DEFAULT_SELF_MD
...
async def test_serve_smoke_loads_config_and_runs_shutdown(monkeypatch, tmp_path):
    config_path = tmp_path / "config.toml"
    socket_path = tmp_path / "kotarou.sock"
...
def test_connect_cli_uses_socket_from_config(monkeypatch, tmp_path):
    ...
    fake_cli_tui = types.ModuleType("connectors.channels.cli_tui")

    def _run_tui(socket: str) -> None:
        observed["socket"] = socket

    fake_cli_tui.run_tui = _run_tui  # type: ignore[attr-defined]
    monkeypatch.setitem(sys.modules, "connectors.channels.cli_tui", fake_cli_tui)
...
def test_init_workspace_creates_expected_assets(tmp_path):
    ...
    assert (workspace / "memory" / "memory_system.db").exists()
...
async def test_start_channels_wires_telegram_and_qq(monkeypatch, tmp_path):
    ...
    fake_qq_channel = types.ModuleType("connectors.channels.qq_channel")
    fake_qqbot_channel = types.ModuleType("connectors.channels.qqbot_channel")
    ...
    monkeypatch.setitem(sys.modules, "connectors.channels.qq_channel", fake_qq_channel)
    monkeypatch.setitem(sys.modules, "connectors.channels.qqbot_channel", fake_qqbot_channel)
