...
async def test_shell_tool_runs_directly_by_default(monkeypatch):
    ...
    monkeypatch.setattr(
        "agent_runtime.tools.shell.asyncio.create_subprocess_shell",
...
def test_restricted_shell_blocks_windows_paths(
    ...
    import agent_runtime.tools.shell as shell_mod
...
async def test_shell_tool_cancel_kills_process_group(monkeypatch):
    ...
    monkeypatch.setattr("agent_runtime.tools.shell.asyncio.wait_for", _fake_wait_for)
    killpg_mock = []

    def _fake_kill_process_tree(proc):
        killpg_mock.append((proc.pid, _KILL_SIGNAL))

    monkeypatch.setattr("agent_runtime.tools.shell._kill_process_tree", _fake_kill_process_tree)

    with pytest.raises(asyncio.CancelledError):
        await __import__("agent_runtime.tools.shell", fromlist=["_run"])._run("sleep 10", 5)
...
async def test_task_stop_deletes_log_file(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("agent_runtime.tools.shell._kill_process_tree", lambda *_: None)
...
async def test_task_output_ttl_deletes_log_file(monkeypatch, tmp_path):
    ...
    killed = []
    monkeypatch.setattr(
        "agent_runtime.tools.shell._kill_process_tree",
