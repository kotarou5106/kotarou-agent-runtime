...
from agent_runtime.core.proactive_turn import (
    ack_discarded,
    ack_on_success,
    ack_post_guard_fail,
    build_delivery_key,
...
async def test_context_only_marks_state_on_success():
    from tests.proactive_system.conftest import FakeRng
    state = FakeStateStore()
...
async def test_ack_on_success_alert_cited_calls_alert_ack_fn():
    ...
    from agent_runtime.core.proactive_turn import ack_on_success
    ctx = AgentTickContext()
    ctx.fetched_alerts = [{"id": "a1", "ack_server": "alert-mcp"}]
    ctx.fetched_contents = []
    ctx.cited_item_ids = ["alert-mcp:a1"]
...
def test_tool_schemas_have_openai_format():
    ...
    from proactive_system.tools import TOOL_SCHEMAS
