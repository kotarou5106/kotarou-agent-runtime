...
import pytest
import agent_runtime.mcp.client as mcp_client_module

from agent_runtime.mcp.client import McpClient, _infer_cwd
from agent_runtime.tool_runtime import append_tool_result
from agent_runtime.tools.base import ToolResult
from agent_runtime.tools.filesystem import (
    EditFileTool,
    ListDirTool,
    ReadFileTool,
    WriteFileTool,
...
from agent_runtime.tools.vision import _encode_image_data_uri
...
async def test_filesystem_tools_cover_core_paths(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    ...
    from agent_runtime.tools import filesystem as _fs_mod
...
def test_vision_rejects_image_when_compression_still_exceeds_limit(
    ...
    from PIL import Image
    from agent_runtime.tools import vision
...
async def test_ipc_server_channel_covers_connection_command_and_response(
    ...
    channel = IPCServerChannel(bus, str(tmp_path / "agent_runtime.sock"), None)

    server = SimpleNamespace(close=MagicMock(), wait_closed=AsyncMock())
    chmod = MagicMock()
    monkeypatch.setattr("connectors.channels.ipc_server.os.chmod", chmod)
    if sys.platform == "win32":
        monkeypatch.setattr(
            "connectors.channels.ipc_server.asyncio.start_server",
            AsyncMock(return_value=server),
        )
    else:
        monkeypatch.setattr(
            "connectors.channels.ipc_server.asyncio.start_unix_server",
            AsyncMock(return_value=server),
        )
    await channel.start()
    if sys.platform == "win32":
...
async def test_ipc_server_uses_tcp_for_explicit_host_port_on_all_platforms(
    ...
    monkeypatch.setattr("connectors.channels.ipc_server.asyncio.start_server", start_server)
    if hasattr(asyncio, "start_unix_server"):
        monkeypatch.setattr("connectors.channels.ipc_server.asyncio.start_unix_server", start_unix_server)
    monkeypatch.setattr("connectors.channels.ipc_server.os.chmod", chmod)
...
async def test_mcp_client_and_loop_factory_cover_core_paths(
    ...
    monkeypatch.setattr("agent_runtime.mcp.client.asyncio.create_subprocess_exec", AsyncMock(return_value=proc))
