...
TraceReplayLog = getattr(_observe_events, "TraceReplayLog")
...
_write_trace_replay = getattr(_observe_writer, "_write_trace_replay")
...
def test_write_trace_replay_persists_timeline_payload(tmp_path):
    db_path = tmp_path / "observe.db"
    conn = open_db(db_path)
    try:
        _write_trace_replay(
        ...
        row = conn.execute(
            """
            select trace_id, session_key, phase, event_type, payload_json
            from trace_events
            where trace_id = ?
            """,
            ("trace-1",),
    ...
    assert row[2] == "prompt"
    assert row[3] == "prompt.rendered"
    assert json.loads(row[4])["messages"][0]["content"] == "hi"
