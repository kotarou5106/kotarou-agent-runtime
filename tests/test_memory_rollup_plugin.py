...
def test_memory_rollup_generates_and_commits_pending(tmp_path: Path):
    memory_system = MemoryStore2(tmp_path / "memory" / "memory_system.db")
    memory_system.upsert_item(
    ...
    with TestClient(create_dashboard_app(tmp_path)) as client:
        ...
        marked_store = MemoryStore2(tmp_path / "memory" / "memory_system.db")
        try:
...
def test_memory_rollup_delete_sources_removes_memory_without_pending(tmp_path: Path):
    ...
    with TestClient(create_dashboard_app(tmp_path)) as client:
        ...
        check_store = MemoryStore2(tmp_path / "memory" / "memory_system.db")
        try:
