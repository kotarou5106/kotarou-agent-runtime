...
import agent_runtime.provider as provider_module
...
from agent_runtime.tool_runtime import append_assistant_tool_calls
from connectors.channels.cli import CLIClient, _print_banner
from connectors.channels.group_filter import DefaultGroupFilter, strip_at_segments
from memory_system.models import MemoryItem
from proactive_system.anyaction import AnyActionGate, QuotaStore
from proactive_system.memory_sampler import sample_memory_chunks, split_memory_chunks
...
async def test_provider_chat_and_retry_paths(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("agent_runtime.provider.AsyncOpenAI", lambda **_: fake)
    slept = []

    async def _sleep(sec: float) -> None:
        slept.append(sec)

    monkeypatch.setattr("agent_runtime.provider.asyncio.sleep", _sleep)
...
async def test_mcp_registry_anyaction_and_sampler_cover_core_paths(
    ...
    monkeypatch.setattr("agent_runtime.mcp.registry.McpClient", _Client)
    monkeypatch.setattr("agent_runtime.mcp.registry.McpToolWrapper", _Wrapper)
    tools = MagicMock()
...
async def test_group_filter_and_cli_paths(
    ...
    if sys.platform == "win32":
        monkeypatch.setattr(
            "connectors.channels.cli.asyncio.open_connection",
            AsyncMock(return_value=(reader, writer)),
        )
    else:
        monkeypatch.setattr(
            "connectors.channels.cli.asyncio.open_unix_connection",
            AsyncMock(return_value=(reader, writer)),
        )
    lines = iter(["hello\n", "exit\n"])
    ...
    monkeypatch.setattr("connectors.channels.cli._read_line", _fake_read_line)
    ...
    _print_banner()
    assert "Kotarou Agent CLI" in capsys.readouterr().out
...
async def test_bootstrap_trigger_and_entrypoints_cover_paths(
    ...
    monkeypatch.setattr(
        "agent_runtime.config.Config.load",
        classmethod(
