...
from storage.sessions.manager import (
    Session,
    SessionManager,
...
async def test_memory_optimizer_loop_and_memory_port_cover_paths(tmp_path: Path):
    ...
    memory.read_self.return_value = "# Kotarou 的自我认知\n## 人格与形象\n- x"
...
async def test_proactive_loop_wrapper_methods_cover_paths(tmp_path: Path):
    ...
    with pytest.MonkeyPatch.context() as mp:
        mp.setattr("proactive_system.loop.compute_energy", lambda last_user_at: 0.8)
        mp.setattr("proactive_system.loop.d_energy", lambda energy: 0.5)
        mp.setattr("proactive_system.loop.next_tick_from_score", lambda *args, **kwargs: 7)
