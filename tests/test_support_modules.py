...
from agent_runtime.context import ContextBuilder, ContextRequest
from agent_runtime.prompting import SYSTEM_CONTEXT_FRAME_MARKER
...
from agent_runtime.tools.registry import ToolMeta, ToolRegistry
...
from agent_runtime.core.common import timekit
from plugins.default_memory.engine import DefaultMemoryEngine
from connectors.persistence.json_store import atomic_save_json, load_json, save_json
from memory_system.memorizer import Memorizer
...
def test_context_builder_builds_prompt_messages_and_assistant_blocks(
    ...
    monkeypatch.setattr("agent_runtime.context.SkillsLoader", _Skills)
    monkeypatch.setattr(
        "agent_runtime.context.build_agent_static_identity_prompt", lambda **_: "identity"
    )
    monkeypatch.setattr(
        "agent_runtime.context.build_telegram_rendering_prompt", lambda: "\ntelegram prompt"
    )
    monkeypatch.setattr(
        "agent_runtime.context.build_skills_catalog_prompt", lambda text: f"catalog:{text}"
    )
...
def test_context_builder_reproduces_temporal_conflict_baseline(
    ...
    monkeypatch.setattr("agent_runtime.context.build_telegram_rendering_prompt", lambda: "")
    monkeypatch.setattr("agent_runtime.context.build_skills_catalog_prompt", lambda text: text)
...
async def test_loop_trigger_and_main_entry_cover_paths(
    ...
    monkeypatch.setitem(
        sys.modules, "connectors.channels.cli_tui", SimpleNamespace(run_tui=MagicMock())
    )
    module.connect_cli("config.toml")
    sys.modules["connectors.channels.cli_tui"].run_tui.assert_called_once_with("/tmp/sock")

    real_import = __import__

    def _fake_import(name, *args, **kwargs):
        if name == "connectors.channels.cli_tui":
            raise RuntimeError("bad tui")
        return real_import(name, *args, **kwargs)

    monkeypatch.delitem(sys.modules, "connectors.channels.cli_tui", raising=False)
    monkeypatch.setattr("builtins.__import__", _fake_import)
    cli_run = AsyncMock()
    monkeypatch.setitem(
        sys.modules,
        "connectors.channels.cli",
