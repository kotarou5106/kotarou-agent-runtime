...
def _import_cli_tui(monkeypatch: pytest.MonkeyPatch):
    ...
    sys.modules.pop("connectors.channels.cli_tui", None)
    return importlib.import_module("connectors.channels.cli_tui")
...
def _import_telegram_channel(monkeypatch: pytest.MonkeyPatch):
    ...
    sys.modules.pop("connectors.channels.telegram_channel", None)
    return importlib.import_module("connectors.channels.telegram_channel")
...
def _import_qq_channel(monkeypatch: pytest.MonkeyPatch):
    ...
    sys.modules.pop("connectors.channels.qq_channel", None)
    return importlib.import_module("connectors.channels.qq_channel")
...
async def test_cli_tui_paths(monkeypatch: pytest.MonkeyPatch):
    ...
    assert mod._env_bool("kotarou_TUI_MOUSE", True) is True
    monkeypatch.setenv("kotarou_TUI_MOUSE", "0")
    assert mod._env_bool("kotarou_TUI_MOUSE", True) is False
...
async def test_qqbot_channel_text_paths(monkeypatch: pytest.MonkeyPatch):
    sys.modules.pop("connectors.channels.qqbot_channel", None)
    mod = importlib.import_module("connectors.channels.qqbot_channel")
    bus = _Bus()
    session_manager = _SessionManager()
