...
from agent_runtime.events.events_lifecycle import TurnCommitted
from agent_runtime.config_models import Config, MemoryConfig
from agent_runtime.tools.registry import ToolRegistry
from bootstrap.memory import build_memory_runtime
from plugins.default_memory.engine import DefaultMemoryEngine
from agent_runtime.core.memory.engine import (
    EngineProfile,
    MemoryCapability,
    MemoryIngestRequest,
    MemoryMutation,
...
from agent_runtime.core.memory.events import ConsolidationCommitted, TurnIngested
from agent_runtime.core.memory.markdown import (
    ConsolidateRequest,
    ConsolidateResult,
    _ConsolidationDraft,
...
from agent_runtime.core.memory.plugin import MemoryPluginRuntime
...
def _make_default_engine(
    *,
    ...
    engine.closeables = []
    engine._wire_memory_system_events()
    return engine
...
def test_build_memory_runtime_exposes_default_memory_engine(
    ...
    monkeypatch.setattr("agent_runtime.memory.MemoryStore", _MemoryStore)
    monkeypatch.setattr("agent_runtime.skills.SkillsLoader", _SkillsLoader)
    monkeypatch.setattr("agent_runtime.tools.memorize.MemorizeTool", _MemorizeTool)
    monkeypatch.setattr("agent_runtime.tools.filesystem.WriteFileTool", _WriteFileTool)
    monkeypatch.setattr("agent_runtime.tools.filesystem.EditFileTool", _EditFileTool)
    monkeypatch.setattr("memory_system.store.MemoryStore2", _Store2)
    monkeypatch.setattr("memory_system.embedder.Embedder", _Embedder)
    monkeypatch.setattr("memory_system.memorizer.Memorizer", _Memorizer)
    monkeypatch.setattr("memory_system.retriever.Retriever", _Retriever)
    monkeypatch.setattr("memory_system.procedure_tagger.ProcedureTagger", _ProcedureTagger)
