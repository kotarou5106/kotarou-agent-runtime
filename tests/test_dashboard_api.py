...
from dashboard.backend.api import create_dashboard_app as _create_dashboard_app
...
class _DashboardMemoryAdmin:
    def __init__(self, workspace) -> None:
        self._store = MemoryStore2(workspace / "memory" / "memory_system.db")
...
def _seed_workspace(tmp_path) -> None:
    ...
    memory_store = MemoryStore2(tmp_path / "memory" / "memory_system.db", vec_dim=2)
    ...
    source = tmp_path / "docs" / "rag.md"
    source.parent.mkdir(parents=True, exist_ok=True)
    source.write_text("# RAG\n\nDocument RAG chunk.", encoding="utf-8")
    knowledge_store.upsert_document_with_chunks(
    ...
    knowledge_store.log_retrieval_event(
        event_id="kretr_test",
        query="Document RAG",
        retrieved_chunk_ids=["kchunk_test"],
        injected_chunk_ids=["kchunk_test"],
    )
...
def test_list_memory_items_sorts_by_created_at_desc(tmp_path) -> None:
    _seed_workspace(tmp_path)
    conn = sqlite3.connect(tmp_path / "memory" / "memory_system.db")
    try:
        conn.execute(
...
def test_knowledge_dashboard_endpoints(tmp_path) -> None:
    _seed_workspace(tmp_path)
    with TestClient(create_dashboard_app(tmp_path)) as client:
        documents = client.get("/api/dashboard/knowledge/documents")
        assert documents.status_code == 200
        payload = documents.json()
        assert payload["total"] == 1
        assert payload["vector_backend"] in {"sqlite-vec", "json-cosine"}
        assert payload["items"][0]["id"] == "kdoc_test"

        chunks = client.get(
            "/api/dashboard/knowledge/chunks",
            params={"document_id": "kdoc_test"},
        )
        assert chunks.status_code == 200
        assert chunks.json()["items"][0]["id"] == "kchunk_test"

        events = client.get("/api/dashboard/knowledge/retrieval-events")
        assert events.status_code == 200
        assert events.json()["items"][0]["injected_chunk_ids"] == ["kchunk_test"]
