...
class ProbePaths:
    ...
    def socket(self) -> Path:
        return self.profile_dir / "kotarou.sock"
    ...
    def memory_db(self) -> Path:
        return self.workspace / "memory" / "memory_system.db"
...
def _run_compose(paths: ProbePaths, args: list[str]) -> None:
    env = {"KOTAROU_DEBUG_PROFILE": paths.profile}
    _ = subprocess.run(
...
async def _run_probe(args: argparse.Namespace) -> None:
    ...
    try:
        if args.reset_workspace:
            _run_compose(
                paths,
                ["run", "--rm", "kotarou-debug", "reset-workspace"],
            )
        if args.start_agent:
            proc = subprocess.Popen(
                ...
                env={
                    **dict(os.environ),
                    "KOTAROU_DEBUG_PROFILE": paths.profile,
                },
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 scripts/docker/debug 沙盒上下文连续性探针。"
    )
