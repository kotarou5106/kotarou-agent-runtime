...
CONFIG="${KOTAROU_DEBUG_CONFIG:-/sandbox/config.toml}"
WORKSPACE="${KOTAROU_DEBUG_WORKSPACE:-/sandbox/workspace}"
SOCKET="/sandbox/kotarou.sock"
DASHBOARD_HOST="${KOTAROU_DASHBOARD_HOST:-0.0.0.0}"
DASHBOARD_PORT="${KOTAROU_DASHBOARD_PORT:-2236}"
HOST_UID="${KOTAROU_HOST_UID:-1000}"
HOST_GID="${KOTAROU_HOST_GID:-1000}"
...
case "$cmd" in
    ...
    run|gateway|serve)
        if [ ! -f "$CONFIG" ]; then
            echo "缺少 $CONFIG，请先运行：docker compose -f scripts/docker/debug/docker-compose.yml run --rm kotarou-debug setup" >&2
            exit 2
        fi
        ensure_socket_config
