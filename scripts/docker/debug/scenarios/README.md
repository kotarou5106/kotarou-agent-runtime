...
这个目录存放可公开的 `scripts/docker/debug/context_probe.py` 输入场景。
...
场景 JSON 是公开测试输入，可以提交。`scripts/docker/debug/profiles/<profile>/workspace/` 下生成的报告 JSON / Markdown 是运行产物，默认不提交。

场景只描述输入和流程，不写结果要求。运行后观察：

- 最终回答是否自然带入前文状态
- `RECENT_CONTEXT.md` 写了什么
- `memory_system.db` 写了什么
- 是否调用了 `recall_memory`
- 普通 history 还剩多少
